...
from models.enums import AccessRequestStatus, ProxyAccessStatus, ProxyAccessType
...
class DashboardSnapshot:
    ...
    audit_count_24h: int
    audit_count_7d: int
    recent_audit_entries: list[dict[str, object]]
...
class DashboardService:
    ...
    async def build_snapshot(self) -> DashboardSnapshot:
        ...
        (
            (users_by_role, new_7d, new_30d, users_active_keys, pending_access, pending_trial),
            (keys_summary, traffic, top_users, proxy_type_counts, audit_24h, audit_7d),
            (recent_audit, ann_30d, all_modules, last_backup, warp_state),
            ...
            asyncio.gather(
                ...
                self._proxy_accesses.count_by_type_status(),
        ...
        # Compute proxy counters from the full type→status breakdown so that
        # SOCKS5 failures are included alongside MTProto failures.
        stuck_statuses = {ProxyAccessStatus.APPLY_FAILED, ProxyAccessStatus.REVOKE_FAILED}
        stuck_proxies = sum(
            per_status.get(s, 0)
            for per_status in proxy_type_counts.values()
            for s in stuck_statuses
        )
        active_socks5 = proxy_type_counts.get(ProxyAccessType.SOCKS5, {}).get(
            ProxyAccessStatus.ACTIVE, 0
        )
        active_mtproto = proxy_type_counts.get(ProxyAccessType.MTPROTO, {}).get(
            ProxyAccessStatus.ACTIVE, 0
        )
