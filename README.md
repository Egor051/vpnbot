...
> **XHTTP fallback topology.** VLESS (HTTP) has no public port of its own: `vless-in`
> (`:443`) terminates REALITY and forwards via a **default catch-all** `fallback`
> (`{ "dest": 8001, "xver": 0 }`, no `path`) to a loopback XHTTP inbound
> (`security: none`); the path is validated on that inbound. A path-based fallback does
> **not** match HTTP/2 XHTTP — the h2 `:path` lives in HPACK, not the request-line — so the
> catch-all is mandatory. The topology is seeded once, by hand; see
