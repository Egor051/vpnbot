...
`AllowedIPs`. **Чей исходящий IP скрывать — определяете вы через
`AllowedIPs`**: укажите диапазоны адресов приложений-шпионов, чей трафик должен
выходить с endpoint туннеля, а не с реального IP сервера. `AllowedIPs` никогда не
изменяется: install-хелпер дословно извлекает его в
`/etc/amnezia/out-warp-routes.list` (один CIDR на строку), а routes-хелпер читает
оттуда.
