...
отзывом/удалением ключей, доступом к прокси SOCKS5 и MTProto, записями аудита и базовой
статистикой трафика — рассчитан на развёртывание на одном сервере без Docker, Redis,
PostgreSQL и тяжёлых ORM.
...
- Одобренные пользователи могут создавать свои ключи Xray/AWG, просматривать свои активные конфигурации и статистику, редактировать заметки к своим ключам.
- Одобренные пользователи могут получать и просматривать свой SOCKS5/MTProto proxy access при включённом backend.
- Revoke/delete для Xray, AWG, SOCKS5 и MTProto — **только для администраторов**. Обычные пользователи не видят кнопки отзыва/удаления, а прямые callback/service-вызовы отклоняются.
