...
- Панель администратора: заявки, пользователи, выдача ключей, аудит, статистика, объявления, диагностика backend, модули протоколов и панель WARP.
...
- Создание ключей Hysteria2 (apernet v2), доставка ссылки, отзыв и удаление **без перезапуска data plane**: отдельный HTTP-эндпоинт `hy2_auth` авторизует handshake'и по живой базе, поэтому отзыв вступает в силу на следующем handshake. По умолчанию выключено — см. [Развёртывание](docs/deployment.ru.md).
...
- Режим обслуживания: суперадмин может из админ-панели перевести бота в обслуживание — все, кроме суперадминов, получают баннер обслуживания, а их апдейты отбрасываются; состояние сохраняется (`maintenance_settings`) и переживает рестарт.
- Панель статуса сервера в реальном времени (CPU, диск, сеть, онлайн-клиенты) с опциональным переключателем детального вида.
- Роли: суперадмины (из `ADMIN_IDS`) плюс внутрибот-роль **модератора** для повседневного управления пользователями/заявками — см. [Политику жизненного цикла доступа](#политика-жизненного-цикла-доступа).
...
- Хранилище SQLite, инициализируемое из `db/schema.sql` (базовые таблицы), с версионными миграциями, применяемыми программно при старте, и `db/indexes.sql`, переприменяемым после них; ротируемые локальные логи и развёртывание через systemd.
- Фоновые задачи: проверка истечения ключей, сбор статистики трафика, health-проба backend Hysteria2, обнаружение аномалий, плановые объявления, зашифрованные офсайтовые бэкапы.
...
flowchart TD
    ...
    HY2AUTH["hy2_auth — авторизация handshake Hysteria2<br/>(отдельный процесс)"] --> DB
...
- `repositories/` — единственный слой, который трогает SQLite; `adapters/` — единственный, который трогает Xray/AWG/Dante/MTProxy/Hysteria2/systemd, в non-root режиме через sudo-хелперы.
- `warp/` — опциональный самодостаточный модуль (туннель, маршруты, split, health-монитор).
- `hy2_auth/` — отдельный процесс (не бот), который авторизует handshake'и Hysteria2 по живой базе через loopback и продолжает работать, пока бот выключен — см. [Hysteria2](docs/hysteria.ru.md).
...
Xray VLESS Reality · AmneziaWG / WireGuard-совместимые инструменты · Hysteria2 (apernet v2).
...
- Для **Hysteria2**-ключей (опционально): сервер **hysteria** (apernet v2) плюс отдельный эндпоинт авторизации handshake `hy2_auth`. Выключено по умолчанию — см. [Hysteria2](docs/hysteria.ru.md).
...
db/schema.sql              # Базовые таблицы и seed-строки (до миграций)
db/indexes.sql             # Все индексы (после миграций)
...
hy2_auth/                  # Отдельный процесс: HTTP-эндпоинт авторизации handshake'ов Hysteria2
scripts/                   # sudo-хелперы WARP (vpn-bot-warp-*) и watchdog warp-failsafe
...
запуска обязательны только `BOT_TOKEN` и `ADMIN_IDS`; заполните соответствующие поля Xray, AWG
или Hysteria2 перед выдачей ключей нужного типа.

| Переменная | Обязательна | Назначение |
|---|---|---|
| `BOT_TOKEN` | **Да** | Токен Telegram Bot API из BotFather. 🔒 |
| `ADMIN_IDS` | **Да** | Telegram user ID **суперадминов** через запятую. Дополнительные модераторы назначаются в боте суперадмином. |
...
**Роли.** Доступ разделён на уровни:

- **Суперадмин** — каждый ID из `ADMIN_IDS`. Полный контроль, включая назначение/снятие роли модератора и любые действия с бэкендами и пользователями.
- **Модератор** — назначается суперадмином из админ-панели (переменной окружения для этого нет). Может одобрять/отклонять заявки на доступ, блокировать/разблокировать и вести пользователей, но **не может** назначать/снимать роль модератора, блокировать другого модератора или суперадмина, а также управлять ключами/прокси чужих владельцев (это остаётся за суперадмином или самим владельцем).
- **Одобренный пользователь** — выдаёт и ведёт свои ключи/прокси-доступ.
- **Ожидающий** — ждёт одобрения; **Заблокированный** — доступ к боту отозван.
...
- Блокировка пользователя — действие модератора/администратора: блокирует доступ к боту и пытается отозвать активные/проблемные VPN-ключи и proxy access (модератор не может заблокировать другого модератора или суперадмина).
...
| [docs/hysteria.ru.md](docs/hysteria.ru.md) | Hysteria2 (apernet v2): data plane, эндпоинт `hy2_auth`, Traffic Stats API, health и восстановление. |
