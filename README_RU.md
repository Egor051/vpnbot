...
| `XRAY_STATS_INTERVAL` | `60` | Интервал (сек) фонового сбора статистики трафика Xray (0–3600; 0 отключает). `statsquery` вызывается без `-reset` (не сбрасывает счётчики), поэтому ручные просмотры статистики опрашивают API напрямую; этот цикл лишь поддерживает кэш свежим между ними, чтобы дашборд обновлялся без действий пользователя. |
...
3. Фоновая asyncio-задача пингует туннель каждые 10 с, учащая до 3 с, как только проба
   осталась без ответа. После **60 с** непрерывного отсутствия ответа маршруты снимаются
   (трафик → напрямую); после **60 с** непрерывного успеха — восстанавливаются.
...
| `WARP_MONITOR_FAIL_WINDOW_SECONDS` | `60` | Сколько секунд **подряд** не должно быть ответа, прежде чем монитор объявит туннель упавшим и уведомит админов. Один успешный пинг сбрасывает окно, поэтому одна потерянная ICMP-проба не вызывает ложную тревогу. |
| `WARP_MONITOR_RECOVER_WINDOW_SECONDS` | `60` | Сколько секунд **подряд** должно быть успешных проб, прежде чем монитор объявит туннель восстановленным. Один провал сбрасывает окно. |
| `WARP_MONITOR_INTERVAL_SECONDS` | `10` | Интервал пинга при нормальной работе. |
| `WARP_MONITOR_FAST_INTERVAL_SECONDS` | `3` | Учащённый интервал пинга, как только проба осталась без ответа, чтобы быстро обнаружить простой (и начало восстановления). |
...
### Управление split-листом из Telegram-бота

После активации `vpnbot-warp-split` администратор может управлять списком
`/etc/vpnbot/warp-split.list` прямо из бота (команды доступны только суперадмину).

#### GUI (кнопки)

В разделе **WARP-туннель** админ-панели есть кнопка **🌐 Split-маршруты** — она
открывает панель управления списком целиком на инлайн-кнопках, не требуя команд:

- заголовок показывает число префиксов; список постранично (≈8 на страницу), у
  каждой строки кнопка **🗑** для удаления;
- **➕ Добавить** — бот просит прислать один или несколько IPv4 CIDR (через
  пробел, запятую или перенос строки); ответ разбирается построчно и выводится
  отчёт (добавлено / дубликат / отклонено с причиной / скорректированы host-биты);
- удаление префикса требует подтверждения **✅ Да / ❌ Нет**;
- **🔄 Применить** — переприменяет текущий список (recover после ручных правок);
- **⬅ Назад в WARP** возвращает в раздел.

GUI — это только представление поверх того же `WarpSplitManager`: все изменения,
guard-проверки и отказ «удаление опустошит список» работают идентично командам.
Каждый callback и шаг FSM проверяет суперадмина на сервере. Команды ниже остаются
полностью рабочими.
...
| Команда | Описание |
|---|---|
| `/warp_split_list` | Показать текущий список (отсортировано + счётчик) |
| `/warp_split_add <cidr…>` | Добавить один или несколько IPv4 CIDR |
| `/warp_split_del <cidr…>` | Удалить CIDR из списка |
| `/warp_split_reload` | Переприменить текущий файл (recover после ручных правок) |

#### Формат ввода для add/del

- Токены разделяются пробелами, запятыми или переводами строк.
- **Маска обязательна** — голый IP (`1.2.3.4`) отклоняется; укажи `/32` явно.
- Только IPv4; IPv6 и доменные имена отклоняются.
- Host-биты исправляются автоматически (`1.2.3.4/24 → 1.2.3.0/24`) с уведомлением.
- Дубли в текущем листе или в рамках батча пропускаются.
- `add` и `del` применяют изменения **немедленно** (один вызов хелпера → один рестарт).

#### Guard-лист (отклоняется ботом)

| Сеть | Причина |
|---|---|
| `0.0.0.0/0` | Используй тумблер WARP-модуля для full-tunnel |
| `10.0.0.0/24` (AWG_NETWORK) | Подсеть VPN-клиентов — нарушит маршрутизацию AWG |
| `172.16.0.0/12` | Туннельная сеть WARP — создаст петлю |
| `127.0.0.0/8` | Loopback |
| `169.254.0.0/16` | Link-local |
| `224.0.0.0/4` | Multicast |
| Подсеть eth0 | Детектируется runtime; удалит доступ к серверу |

> **Удаление до пустого списка отклоняется.** Для перехода на full-tunnel:
> `sudo systemctl disable --now vpnbot-warp-split`. Для all-direct: оставь
> sentinel-префикс (напр. `192.0.2.0/24` — TEST-NET, нигде не маршрутизируется).

#### Архитектура (принцип наименьших привилегий)

Бот **только читает** файл напрямую (0644) и **пишет исключительно через**
`vpnbot-warp-split-apply` (root:root 0755, sudo-грант строго на этот бинарь).
Хелпер: валидирует каждый CIDR (`python3 ipaddress`), записывает атомарно
(temp + rename), перезапускает `vpnbot-warp-split`. Бот никогда не вызывает
`ip`/`iptables`/`awg-quick` напрямую.

#### Кнопки Вкл/Выкл/Перезапустить (управление РОУТАМИ)

Кнопки **Включить / Выключить / Перезапустить** в панели «Сокрытие outbound IP»
управляют именно split-**маршрутами** в динамической таблице `T`, а НЕ туннелем:
интерфейс `out-warp` и процесс `awg-quick@out-warp` остаются под управлением
systemd (observer-модель), бот их не трогает.

- **Выключить** — реконсайл таблицы `T` в пусто: снимаются все per-prefix
  маршруты `<prefix> dev out-warp`, весь клиентский/прокси-трафик идёт напрямую
  (direct). Сохранённый список `/etc/vpnbot/warp-split.list` **не стирается**, а
  анти-луп `162.159.195.1/32 via eth0-gw`, `ip rules` и NAT/FORWARD не трогаются.
...
`/etc/vpnbot/warp-split.disabled`, который `vpnbot-warp-split` учитывает на каждом
boot-apply — поэтому «выключено» переживает перезагрузку. Все мутации таблицы `T`
проходят через `vpnbot-warp-split-state` (sudo-грант строго на вербы
`on|off|restart|status`, без wildcard). Строки «Туннель» (observer) и «Маршруты»
(намерение маркера + фактическая таблица `T`) в панели берутся из `status()`;
при рассинхроне отображается предупреждение, статус не падает ни в одном состоянии.

Точка входа в GUI списка split-префиксов (➕/🗑/🔄) переехала из главной
WARP-панели в под-панель **«Настройки WARP»** (рядом с «Заменить/Удалить конфиг»);
сами команды `/warp_split_*` и логика списка не изменились.
