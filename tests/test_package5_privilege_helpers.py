...
def test_socks5_helper_rejects_unsafe_login_names(login: str) -> None:
...
def test_socks5_helper_accepts_valid_managed_login() -> None:
    helper = _load_helper("vpnbot-socks5-user")

    assert helper.validate_login("vpn_socks_100_abcd") == "vpn_socks_100_abcd"


def test_socks5_set_password_reads_secret_from_stdin_and_uses_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    helper = _load_helper("vpnbot-socks5-user")
...
def test_socks5_set_password_rejects_secret_in_argv() -> None:
    helper = _load_helper("vpnbot-socks5-user")
...
def test_socks5_helper_error_output_redacts_password(monkeypatch: pytest.MonkeyPatch) -> None:
    helper = _load_helper("vpnbot-socks5-user")
...
def test_xray_helper_rejects_candidate_outside_staging_root(tmp_path: Path) -> None:
...
def test_xray_helper_rejects_symlink_candidate(tmp_path: Path) -> None:
...
def test_xray_helper_uses_fixed_target_service_and_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    helper = _load_helper("vpnbot-xray-apply")
    calls: list[list[str]] = []
    ...
    assert calls == [[helper.XRAY_BIN, "run", "-test", "-config", str(Path("/run/vpn-bot/xray/candidate.json"))]]


def test_awg_helper_rejects_candidate_outside_staging_root(tmp_path: Path) -> None:
...
def test_awg_helper_uses_fixed_target_and_interface() -> None:
    helper = _load_helper("vpnbot-awg-apply")
...
def test_awg_helper_redacts_private_config_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(helper, "quick_strip", lambda path: (_ for _ in ()).throw(helper.HelperError("AWG config validation failed")))
    stderr = io.StringIO()
...
def test_mtproxy_helper_rejects_candidate_outside_staging_root(tmp_path: Path) -> None:
    helper = _load_helper("vpnbot-mtproxy-apply")
    helper.STAGING_ROOT = tmp_path / "staging"
...
def test_mtproxy_helper_redacts_managed_secret_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    helper = _load_helper("vpnbot-mtproxy-apply")
    helper.STAGING_ROOT = tmp_path / "staging"
    ...
    (candidate / "managed-secrets.json").write_text(
        json.dumps({"version": 1, "generation": 1, "secrets": [{"secret": secret}], "runtime_secrets": []}),
        encoding="utf-8",
    )
    ...
    monkeypatch.setattr(helper, "restart_and_verify", lambda port: (_ for _ in ()).throw(helper.HelperError("MTProxy restart failed")))
    stderr = io.StringIO()
...
def test_direct_mode_remains_default(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_xray_adapter_helper_mode_stages_candidate_and_calls_fixed_helper(tmp_path: Path) -> None:
    ...
    class Shell:
        ...
        async def run(self, args: list[str], **kwargs: object) -> ShellResult:
            ...
            assert staged["inbounds"][0]["settings"]["clients"][0]["id"] == "00000000-0000-4000-8000-000000000001"
...
def test_awg_adapter_helper_mode_stages_candidate_and_calls_fixed_helper(tmp_path: Path) -> None:
...
def test_mtproxy_adapter_helper_mode_stages_files_and_calls_fixed_helper(tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        await adapter.apply_managed_secrets([MtProxyManagedSecret(secret=secret, fingerprint="fp-1", owner_user_id=100, access_id=1)])
