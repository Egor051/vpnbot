...
- The installed wrapper is a copy of `deploy/run-mtproxy-managed` (not a symlink), and nothing refreshes it automatically — `git pull`/`git reset` and `scripts/deploy.sh` leave it untouched (it is a gitignored install artifact). After pulling a new release, re-install it so wrapper security fixes actually reach the proxy: `sudo install -m 700 -o root -g root deploy/run-mtproxy-managed /opt/vpn-service/scripts/run-mtproxy-managed && sudo systemctl restart mtproxy`. See the update runbook in [operations.md](operations.md#maintenance--update-from-github).
- Before switching `MTPROTO_MODE=managed` on, confirm `MTPROTO_MANAGED_SECRETS_PATH` in the live `mtproxy.env` points at the directory that actually holds `managed-secrets.json`. The managed directory is `/etc/mtproxy/vpn-bot/` (hyphenated — matching the repo default and `MTPROTO_MANAGED_ENV_PATH`); a legacy non-hyphen path such as `/etc/mtproxy/vpnbot/managed-secrets.json` will not be found and the wrapper will fail to read secrets. Reconcile the spelling on both the env variable and the on-disk file first.
