...
## Клиентские профили транспорта (VLESS (HTTP))

При `XRAY_XHTTP_ENABLED=true` создание ключа **VLESS (HTTP)** добавляет финальный шаг
выбора **клиентского профиля транспорта**. Это чисто **клиентский** выбор: все три
профиля — клиенты одного и того же loopback XHTTP-инбаунда (`mode: auto`), поэтому
**серверная настройка не меняется** — сидинг выше остаётся прежним. Профиль лишь
подстраивает `mode` и блок `extra` (`xhttpSettings`) в генерируемой ссылке VLESS (HTTP)
и **неизменяем** для ключа (сменить профиль = выпустить новый ключ).

| Профиль | Метка ссылки | `mode` | Клиентский тюнинг | Когда использовать |
|---|---|---|---|---|
| **Базовый** (`base`, по умолчанию) | `VLESS (HTTP)` | `XRAY_XHTTP_MODE` (по умолчанию `stream-one`) | нет | Универсальный — лучший баланс скорости и стабильности; ссылка байт-в-байт как до профилей. |
| **Антисибирский** (`antisib`) | `VLESS (HTTP) · 🛡 Антисибирский` | `stream-one` | одноканальный xmux (`maxConnections:1`, `cMaxReuseTimes:64-128`) | Соединение рвётся в момент подключения из-за блокировки по числу TLS-рукопожатий. Один канал — на слабой сети может быть медленнее. |
| **Мультиподключение** (`multi`) | `VLESS (HTTP) · 🔀 Мультиподключение` | `packet-up` | дробление post + ротация xmux (`scMaxEachPostBytes`, `scMinPostsIntervalMs`, `maxConnections:2`, `cMaxReuseTimes:8-16`, `hMaxReusableSecs:30-60`) | Устойчивость долгих сессий: дробит трафик на короткие соединения против ограничения скорости на длинных подключениях. |

> ⚠️ **Версия клиента.** Профиль `multi` использует `hMaxReusableSecs` (ротация xmux
> в секундах), что требует **Xray-core v25.3.6+** на клиенте; на более старых ядрах
> тюнинг ротации молча игнорируется. `base` и `antisib` от него не зависят.

Профиль `base` — по умолчанию и воспроизводит ссылку в точности как до профилей,
поэтому существующие развёртывания и уже выпущенные ключи не затрагиваются.
`XRAY_XHTTP_MODE` задаёт `mode` только для профиля `base`; `antisib`/`multi` несут
собственный режим. Выбранный профиль хранится по ключу (колонка `xhttp_profile`,
миграция v28) и кодируется в Xray email-label ключа (`xray_http_{base,antisib,multi}_<rnd>`).
