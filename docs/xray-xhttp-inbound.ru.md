...
`XRAY_XHTTP_MODE` задаёт `mode` только для профиля `base`; `multi`/`antisib` несут
собственный режим. Выбранный профиль хранится по ключу (колонка `xhttp_profile`,
миграция v28) и кодируется в Xray email-label ключа (`xray_http_{base,multi,antisib}_<rnd>`).
