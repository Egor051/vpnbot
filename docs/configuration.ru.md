...
| `WARP_SPLIT_FEED_ALERT_DELTA_PCT` | `30` | **Автоматическое** обновление применяется само, но при изменении списка больше чем на этот процент шлётся уведомление админам. Ручное обновление всегда показывает preview дельты и требует подтверждения независимо от этого значения. |
