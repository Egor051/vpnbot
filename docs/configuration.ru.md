...
> профиля — **base** / **multi** (мультиподключение) / **antisib** (анти-блокировка), —
...
stats-label ключа (`hy2_<rnd>`), тот же, что возвращает `hy2_auth`.
...
| `SUBSCRIPTION_PUBLIC_PORT` | Нет | `0` | Публичный HTTPS-порт того же процесса (TLS терминирует сам aiohttp — reverse-proxy в стеке нет). Пусто или `0` выключает публичный слушатель. Если порт задан, **обе** TLS-переменные ниже обязательны: без них процесс fail-closes и не стартует. Порт открывается через `deploy/ufw-subscription.sh`. |
...
VLESS (TCP), каждый профиль VLESS (HTTP) (`base`, `multi`, `antisib`) и
...
**Алерт аномалии по ребёнку бандла называет бандл.** Детект не менялся — он
по-прежнему сэмплит по дочерним ключам через их email-метки, — но админу
приходит алерт с id и отображаемой меткой бандла, а не с внутренней меткой
ребёнка, которую пользователь никогда не видел. Дети **одного** бандла,
перешедшие порог в одном окне, сворачиваются в **один** алерт с объединённой
IP-картиной, например `бандл #7 · затронуто 3 ключа (VLESS TCP, VLESS HTTP,
HYSTERIA2) · 7 уник. сетей за последние 60 мин`. Это не только про уменьшение
шума: расшаренный sub-URL зажигает сразу несколько протоколов одним и тем же
набором IP, поэтому свёрнутый алерт прямо формулирует признак шаринга вместо
того, чтобы размазать его по алерту на протокол. Алерты по одиночным ключам не
изменились.
...
`SUBSCRIPTION_ENABLED` гейтит и **UI бота**: пока он `false`, в «Создать ключ»
нет опции all-in-one, в «Мои ключи» не показываются бандлы, а callback `bundle:*`
отклоняется. У sub-URL, который показывает бот, **нет своей переменной для
host** — это `HYSTERIA2_SNI` (с откатом на `HYSTERIA2_HOST`) плюс
`SUBSCRIPTION_PUBLIC_PORT`, потому что эндпоинт терминирует TLS копией
сертификата этого же домена. При `SUBSCRIPTION_PUBLIC_PORT=0` эндпоинт доступен
только по loopback, и экран конфига честно об этом сообщает вместо нерабочей
ссылки. Оба процесса читают `.env` при старте, поэтому переключение флага
требует `systemctl restart vpn-bot-subscription vpn-bot`.
...
| `WARP_SPLIT_FEEDS_INTERVAL_SEC` | `0` | Период автообновления источников префиксов, в секундах. **`0` (по умолчанию) полностью выключает планировщик** — подсистема источников работает только по кнопке «Обновить сейчас». Поставь, например, `21600` (6 ч), чтобы включить. См. [источники префиксов](warp.ru.md#источники-префиксов-автообновление-списка). |
| `WARP_SPLIT_FEED_TIMEOUT_SEC` | `20` | Полный HTTP-таймаут одной загрузки (5–300). Таймаут — это ошибка загрузки: берётся последний удачный кэш, итоговый список не меняется. |
| `WARP_SPLIT_FEED_MAX_BYTES` | `2000000` | Ограничение на размер тела ответа. Проверяется по `Content-Length`, если он есть, и повторно по ходу чтения потока — сломанный или враждебный эндпоинт не заставит бот буферизовать безразмерный ответ. |
| `WARP_SPLIT_FEED_CACHE_DIR` | `/var/lib/vpn-bot/warp-feeds` | Куда складывается последнее успешно распарсенное тело каждого источника (каталог 0700, файлы 0600). Именно этот кэш делает неудачное обновление безвредным: без него мёртвый upstream укоротил бы маршрутизируемый список. |
| `WARP_SPLIT_FEEDS_MODE` | `review` | Что прогону **позволено делать** — в отличие от того, как часто он случается: `off` (не применяет ничего), `review` (любое изменение, включая прошедшее все пороги, уходит админам карточкой и применяется только руками), `auto` (применяет прошедшее проверки ниже, остальное — в ту же карточку). `off` отключает применение при любом интервале; интервал `0` отключает прогон при любом режиме. См. [поднадзорное обновление](warp.ru.md#поднадзорное-обновление-review-и-auto). |
| `WARP_SPLIT_MAX_SHRINK_PCT` | `20` | Источник, потерявший больше этой доли своего **адресного** покрытия (относительно последнего фактически применённого состояния), удерживается до подтверждения. Работает и для subtract-источников, причём там направление зеркально: усохший вычитаемый вычитает меньше, значит в туннель попадёт *больше* чужого адресного пространства. |
| `WARP_SPLIT_MAX_GROWTH_PCT` | `50` | Рост **адресного** покрытия итогового списка относительно последнего применённого состояния, выше которого изменение удерживается до подтверждения. Значения больше 100 законны (включение второго источника может больше чем удвоить список). |
| `WARP_SPLIT_CONFIRM_STREAK` | `2` | Только для `auto`: сколько прогонов подряд должны дать *один и тот же* результат, прежде чем он применится. Защита от разовой битой публикации, которую провайдер починит через час. Чистая переагрегация (покрытие то же, число префиксов другое) от подтверждения освобождена. |
| `WARP_SPLIT_FEED_STALE_AFTER_SEC` | `259200` | Источник, чья последняя удачная загрузка старше этого срока, делает всё состояние «неполным»: кандидат уходит в очередь на подтверждение, а не применяется по копии, которую никто не смог подтвердить. По умолчанию 3 суток. |
| `WARP_SPLIT_ALERT_COOLDOWN_SEC` | `3600` | Минимальный промежуток между двумя одинаковыми уведомлениями об одном источнике. Журнал дедупликации живёт **в памяти**, поэтому перезапуск бота может повторить одно сообщение; на карточки подтверждения cooldown не распространяется. |
| `WARP_SPLIT_FEED_FAIL_STREAK` | `3` | Сколько неудачных загрузок подряд по одному источнику, прежде чем сказать админам. Один таймаут — шум (merge берёт кэш), три прогона подряд — источник встал. |
| `WARP_SPLIT_MAX_PREFIXES` | `1500` | Жёсткий потолок размера итогового списка. Превышение = отказ **целиком** + алерт, никакого частичного применения. Важно потому, что вычитание фрагментирует префиксы (`goog − cloud` превращает 99 префиксов в 262). |
| `WARP_SPLIT_MIN_PREFIXLEN` | `8` | Самая короткая допустимая маска. Всё шире (`/7`, `0.0.0.0/0`, …) отвергается: это гораздо вероятнее опечатка, чем намерение, и завернуло бы в туннель заметную долю интернета. |
