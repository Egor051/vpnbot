...
ослабляет секрет авторизации каждого ключа. Поскольку salamander-обфускация
убрана (чистый QUIC на UDP/443), PSK больше не сужает этот риск: слепой on-path
атакующий может видеть и прощупывать handshake, но не может расшифровать данные
QUIC-приложения или аутентифицироваться без валидного секрета ключа.
