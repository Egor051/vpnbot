...
> самопроверку — убеждается, что хост НЕ в туннеле, и **замеряет счётчики байт `out-warp`**
> (реальный клиентский трафик), а не симулирует путь клиента через `ip route get`, который не
> видит conntrack-метку, — и откатывается к прямому выходу клиентов, только если хост оказался
> захвачен туннелем.
...
- **Выключить** — реконсайл таблицы `T` в пусто: снимаются все per-prefix маршруты `<prefix> dev out-warp` **и дефолт таблицы `default dev out-warp`**, а прямой `MASQUERADE` на WAN для клиентской подсети и proxy-источника плюс FORWARD-разрешения `awg0 <-> <wan>` идемпотентно (пере)ставятся (`-C … || -A/-I`) — весь клиентский/прокси-трафик идёт напрямую и с NAT. Сохранённый список `/etc/vpn-bot/warp-split.list` **не стирается**, анти-луп `162.159.195.1/32 via eth0-gw` и `ip rules` не трогаются.

  Обе половины важны на **boot-пути**: там это выполняется после `awg-quick@out-warp` (`Table=auto` уже поставил `default dev out-warp` в таблицу `T`) и после `vpn-bot-warp-routes` (который сделал swap NAT: снял прямой `-s 10.0.0.0/24 -o <wan> -j MASQUERADE` и добавил `-o out-warp -j MASQUERADE`). Снятие только per-prefix маршрутов оставило бы дефолт туннеля на месте — «выключено» молча означало бы **полный туннель**; а удаление дефолта без восстановления NAT отправило бы клиентов в WAN без маскарада.
