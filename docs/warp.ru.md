...
`vpnbot-warp-*`. Системный DNS-резолвер не трогается. Маршруты по умолчанию (`0.0.0.0/0`, `::/0`)
...
> **Примечание:** хост защищён по дизайну — `vpnbot-warp-routes` снимает host-bypass awg-quick
...
[`../deploy/helpers/README.md`](../deploy/helpers/README.md) и `deploy/sudoers.d/vpnbot.example`):
...
- **Source-bind демоны** (Dante, Xray) биндят source egress на IP туннеля; `vpnbot-warp-routes` добавляет одно правило `ip rule from <tunnel-ip> lookup <T>` и **NAT не нужен** (src уже корректный):
...
> 3. Переустановите конфиг туннеля, чтобы в `[Interface]` был `Table = auto` (`vpnbot-warp-install`).
...
Selective-split направляет через WARP только префиксы из `/etc/vpnbot/warp-split.list`; остальной
...
2. **Установка нового слоя** (запускать из корня репозитория от root):
   ...
   Скрипт устанавливает `vpnbot-warp-split`, `warp-failsafe`, их unit-файлы, перезагружает
...
- **Только selective-split:** `sudo systemctl disable --now vpnbot-warp-split` + reboot → возврат к full-tunnel (весь клиентский трафик идёт через WARP).
...
- **Выключить** — реконсайл таблицы `T` в пусто: снимаются все per-prefix маршруты `<prefix> dev out-warp`, весь клиентский/прокси-трафик идёт напрямую. Сохранённый список `/etc/vpnbot/warp-split.list` **не стирается**, а анти-луп `162.159.195.1/32 via eth0-gw`, `ip rules` и NAT/FORWARD не трогаются.
...
`/etc/vpnbot/warp-split.disabled`, который `vpnbot-warp-split` учитывает на каждом boot-apply —
поэтому «выключено» переживает перезагрузку. Все мутации таблицы `T` проходят через
`vpnbot-warp-split-state` (sudo-грант строго на вербы `on|off|restart|status`, без wildcard).
...
После активации `vpnbot-warp-split` список префиксов можно вести прямо из Telegram, без SSH:
