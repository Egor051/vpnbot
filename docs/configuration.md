...
| `SUBSCRIPTION_PUBLIC_PORT` | No | `0` | Public HTTPS port the same process listens on (aiohttp terminates TLS itself; no reverse proxy in this stack). Empty or `0` disables it. When set, **both** TLS values below are mandatory — without them the process fail-closes and does not start. Open the port in ufw via `deploy/ufw-subscription.sh`. | `2096` |
...
| `WARP_SPLIT_FEEDS_INTERVAL_SEC` | `0` | Seconds between automatic prefix-feed refreshes. **`0` (the default) disables the scheduler entirely** — the feed subsystem then only runs when an admin presses "Refresh now". Set e.g. `21600` (6 h) to enable. See [prefix feeds](warp.md#prefix-feeds-automatic-list-updates). |
| `WARP_SPLIT_FEED_TIMEOUT_SEC` | `20` | Total HTTP timeout for one feed download (5–300). A timeout is a fetch failure: the last good cached body is used and the routed list does not change. |
| `WARP_SPLIT_FEED_MAX_BYTES` | `2000000` | Size cap for a feed body. Enforced from `Content-Length` when present and again while streaming, so a broken or hostile endpoint cannot make the bot buffer an unbounded response. |
| `WARP_SPLIT_FEED_CACHE_DIR` | `/var/lib/vpn-bot/warp-feeds` | Where the last successfully parsed body of each feed is cached (dir 0700, files 0600). This cache is what makes a failed refresh harmless — without it a dead upstream would shorten the routed list. |
| `WARP_SPLIT_FEEDS_MODE` | `review` | What a scheduled run may **do**, as opposed to how often it runs: `off` (applies nothing), `review` (every change — including one that passes every threshold — goes to the admins as a card and is applied only by hand), `auto` (applies what clears the guards below, queues the rest). `off` disables applying at any interval; interval `0` disables the run at any mode. See [supervised refresh](warp.md#supervised-refresh-review-and-auto). |
| `WARP_SPLIT_MAX_SHRINK_PCT` | `20` | A source that loses more than this share of its **address** coverage (versus the last state actually applied) is held for approval. Applies to subtract sources too, where the direction is mirrored: a shrinking subtrahend carves out less, so *more* foreign address space enters the tunnel. |
| `WARP_SPLIT_MAX_GROWTH_PCT` | `50` | Growth of the merged list's **address** coverage, versus the last applied state, above which the change is held for approval. Values above 100 are legitimate (enabling a second feed can more than double the list). |
| `WARP_SPLIT_CONFIRM_STREAK` | `2` | `auto` only: how many consecutive runs must compute the *same* candidate before it is applied, so one broken publication corrected an hour later never reaches the routing table. Pure re-aggregation (identical coverage, different prefix count) is exempt. |
| `WARP_SPLIT_FEED_STALE_AFTER_SEC` | `259200` | A source whose last successful fetch is older than this makes the merged state "incomplete": the candidate is queued for approval rather than applied from a copy nobody has been able to confirm. Default is 3 days. |
| `WARP_SPLIT_ALERT_COOLDOWN_SEC` | `3600` | Minimum gap between two identical alerts about the same source. The ledger is held **in memory**, so a bot restart can repeat one message; approval cards are never suppressed by it. |
| `WARP_SPLIT_FEED_FAIL_STREAK` | `3` | Consecutive failed fetches of one source before the admins are told. One timeout is noise — the merge falls back to the cache; three runs in a row is a feed that has stopped. |
| `WARP_SPLIT_MAX_PREFIXES` | `1500` | Hard ceiling on the merged list. Exceeding it refuses the **whole** operation and alerts — never a partial apply. Relevant because subtraction fragments prefixes (`goog − cloud` turns 99 prefixes into 262). |
| `WARP_SPLIT_MIN_PREFIXLEN` | `8` | Shortest prefix allowed in the list. Anything broader (`/7`, `0.0.0.0/0`, …) is refused: it is far more likely a typo than intent, and it would route a large share of the internet through the tunnel. |
