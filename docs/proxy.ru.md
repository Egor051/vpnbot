...
- В production процесс бота не вызывает инструменты управления аккаунтами напрямую. Он использует `sudo -n /usr/local/sbin/vpn-bot-socks5-user ...`; только хелпер имеет право вызывать `getent`, `useradd`, `chpasswd`, `passwd -l` и `userdel`.
...
- Установленный wrapper — это копия `deploy/run-mtproxy-managed` (не симлинк), и ничто не обновляет его автоматически: `git pull`/`git reset` и `scripts/deploy.sh` его не трогают (это gitignore-артефакт установки). После обновления релиза переустановите его, чтобы security-фиксы wrapper'а реально попали в прокси: `sudo install -m 700 -o root -g root deploy/run-mtproxy-managed /opt/vpn-service/scripts/run-mtproxy-managed && sudo systemctl restart mtproxy`. См. ранбук обновления в [operations.ru.md](operations.ru.md#обслуживание--обновление-с-github).
- Перед включением `MTPROTO_MODE=managed` убедитесь, что `MTPROTO_MANAGED_SECRETS_PATH` в живом `mtproxy.env` указывает на каталог, где реально лежит `managed-secrets.json`. Managed-каталог — `/etc/mtproxy/vpn-bot/` (с дефисом — как репо-дефолт и `MTPROTO_MANAGED_ENV_PATH`); устаревший путь без дефиса вроде `/etc/mtproxy/vpnbot/managed-secrets.json` не будет найден, и wrapper не сможет прочитать секреты. Сначала согласуйте написание и в переменной окружения, и в пути файла на диске.
...
- Во время работы бот размещает MTProxy-кандидаты в `/run/vpn-bot/mtproxy`. Хелпер `/usr/local/sbin/vpn-bot-mtproxy-apply` валидирует эти файлы, записывает `MTPROTO_MANAGED_SECRETS_PATH`, записывает `MTPROTO_MANAGED_ENV_PATH`, поддерживает `MTPROTO_BACKUP_DIR/<backup-id>/`, перезапускает `mtproxy`, проверяет `systemctl is-active`, проверяет, что `MTPROTO_PORT` слушает, и восстанавливает предыдущие managed-файлы при ошибке apply.
...
2. Проверьте `MTPROTO_BACKUP_DIR`, по умолчанию `/etc/mtproxy/vpn-bot/backups`.
