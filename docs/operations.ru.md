...
## Деплой перезапускает и sidecar-юниты

`vpn-bot.service` — не единственный процесс, работающий на выкаченном чекауте. Тот же код
поднимают ещё два юнита, каждый со своим долгоживущим процессом, держащим код в памяти:

| Юнит (откуда берётся имя) | Пакет |
|---|---|
| `vpn-bot-subscription.service` (`SUBSCRIPTION_UNIT`) | `subscription_server/` — `GET /sub/{token}` |
| `vpnbot-hy2-auth.service` (`HYSTERIA2_AUTH_SERVICE_NAME` в `.env`) | `hy2_auth/` — HTTP-авторизация Hysteria2 |

Раньше Phase 2 перезапускала только бота, поэтому любой PR, трогающий `subscription_server/`,
`hy2_auth/`, `services/key_bundles.py` или общие форматтеры ссылок, уезжал **наполовину**: бот
отдавал новый код, sidecar продолжал отдавать старый из памяти, а отчёт деплоя был полностью
зелёным. Это не гипотеза: PR #281 выкатился 2026-08-08 в 13:56, а subscription-эндпоинт отдавал
прежний порядок ссылок без marks до случайной перезагрузки хоста в 15:20.

**Ручного шага рестарта после деплоя больше нет.** Phase 2 перезапускает эти юниты сама — после
того как `vpn-bot` подтверждён активным и пройден гейт владельца shared-WAL БД: sidecar открывают
тот же `vpn.db`, и рестарт поверх БД с неправильным владельцем — это ровно тот рестарт, который
превращает скрытую проблему прав в `unable to open database file`.

Правила намеренно узкие:

- **Решает pre-состояние.** Состояние каждого юнита снимается в Phase 1, до любых мутаций.
  Перезапускается только тот, что был **active** на момент, когда деплой застал хост.
- **Юнита нет → skip, и это не ошибка.** Оба юнита ставятся руками (drift), и деплой
  по-прежнему **не** устанавливает, не enable-ит и не запускает ни один из них. Хост без них —
  нормальный хост: протоколы модульные.
- **Был неактивен → остаётся неактивен.** Деплой не поднимает то, что оператор остановил.
- **Фатален ровно один случай:** был active до деплоя и не вернулся в `active` после нашего
  рестарта. Это регрессия, вызванная именно этим деплоем, и она уходит в обычный rollback.
  Всё остальное — информационно, включая порт, который не отвечает при активном юните.
- **По строке на юнит в отчёте** — `restarted` / `skipped (was inactive)` / `skipped (absent)` —
  и в отчёте `PHASE1_ONLY`, и в финальном блоке `DEPLOY OK`, чтобы половинчатая выкатка была
  видна, а не молчала.

> ⏱️ **Рестарт `vpnbot-hy2-auth` на секунды отбрасывает новые Hysteria2-хендшейки.** Эндпоинт
> намеренно **fail-closed** (`hysteria-server` получает `ok: false`, а не 5xx, когда ответ
> получить нельзя), поэтому каждый хендшейк, пришедший во время рестарта, отклоняется, и клиент
> повторяет попытку. Уже установленные сессии не затрагиваются — только новые подключения в этом
> окне. Ещё одна причина делать мутирующие деплои в спокойное окно; главная — [окно потери данных
> при откате](#rollback-после-неудачного-деплоя).
...
>
> **Sidecar откат не возвращает.** Rollback восстанавливает дерево на `PREV_SHA` и перезапускает
> `vpn-bot`, но [sidecar-юнит](#деплой-перезапускает-и-sidecar-юниты), который деплой уже успел
> перезапустить, продолжает работать на новом коде. Вывод rollback'а называет такие юниты —
> перезапустите их руками, когда дерево вернётся назад.
...
# Sidecar работают на том же дереве из своих юнитов и держат код в памяти.
# `scripts/redeploy.sh` перезапускает их сам; этот ручной путь — нет, поэтому делаем здесь:
# пропустите шаг — и эндпоинты продолжат отдавать код, который вы только что заменили.
sudo systemctl restart vpn-bot-subscription vpnbot-hy2-auth   # только те, что есть на хосте
python deploy/check-nonroot-helper-mode.py
```

**Предпочитайте `scripts/redeploy.sh` этому ручному пути.** Он проверяет, делает бэкап,
откатывается при провале проверки и перезапускает каждый активный
[sidecar-юнит](#деплой-перезапускает-и-sidecar-юниты) — ручной путь выше не делает ничего из
этого.
