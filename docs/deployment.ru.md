...
### 2c. (Опционально) Маскарад-decoy

`deploy/hysteria/config.yaml` также объявляет файловый masquerade:
...
Пробе, которая не завершает Hysteria2-handshake, отдаётся статический сайт из
`/etc/hysteria/masq` — по существующему UDP/443 QUIC-пути. Этот каталог **не** трекается в
репозитории — кладётся на хост вручную, так же как `tls.cert`/`tls.key`; `install-config.sh`
его не трогает. `listenHTTP`/`listenHTTPS` намеренно **не** настроены: они открыли бы отдельные
plaintext TCP-листенеры `:80`/`:443`, а TCP/443 на этом хосте уже занят Xray REALITY.
...
- `scripts/deploy.sh` в Phase 2 **перезапускает этот юнит** на каждом деплое, если до деплоя он
  был активен (имя юнита читается из `HYSTERIA2_AUTH_SERVICE_NAME`, поэтому переименованный юнит
  тоже покрыт), — чтобы изменения в `hy2_auth/` реально дошли до работающего процесса; ручного
  рестарта после деплоя не требуется. Поскольку эндпоинт fail-closed, те несколько секунд, что он
  лежит, **отклоняют все новые Hysteria2-хендшейки**: установленные сессии не затрагиваются и
  клиенты повторяют попытку, но это ещё одна причина деплоить в спокойное окно. Полная политика —
  в [ранбуке](operations.ru.md#деплой-перезапускает-и-sidecar-юниты).
